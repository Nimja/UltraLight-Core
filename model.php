...
abstract class Model
{
    ...
    function __construct($values = null)
    {
        ...
        if (class_exists('Library_Login', false)) {
            $this->_edit = Library_Login::$role > Library_Login::ROLE_NEUTRAL;
        }
        ...
        $this->_table = self::$prefix . strtolower(str_replace(self::CLASS_PREFIX, '', $this->_class));
    ...
    protected function getDefaults()
    {
        ...
        return method_exists('Defaults', $function) ? Library_Defaults::$function() : array();
    }
    ...
    protected function getDb()
    {
        if (empty(self::$db)) {
            if (empty($this->_table)) {
                Show::fatal('Cannot connect to database unless table is defined.', $this->_class);
            }
            if (empty($this->_fields) || !is_array($this->_fields)) {
                Show::fatal('Cannot connect to database unless fields are defined.', $this->_class);
            }

            self::$db = new Library_DataBase();
        }
        return self::$db;
    }
    ...
    protected function check()
    {
        ...
        $db = $this->getDb();
        return $db->table_exists($this->_table);
    }
    ...
    public function install($force = false)
    {
        ...
        if (!$this->check() || $force) {
            $installed = $db->create_table($this->_table, $fields, true);
            ...
            $updated = $db->update_table($this->_table, $fields);
            if ($updated) {
    ...
    public function save()
    {
        ...
        if ($id > 0) {
            ...
            $db->update($this->_table, $values, 'id=' . $id);
        } else {
            $this->id = $db->insert($this->_table, $values);
        }
        $this->_cache[$this->_class . '-' . $this->id] = $this;
        $this->_saved = true;
        return true;
    ...
    protected function load($id)
    {
        ...
        $values = $this->getDb()->run('SELECT * FROM `' . $this->_table . '` WHERE id = ' . $id);
    ...
    public function delete($id = null)
    {
        ...
        $this->getDb()->delete($this->_table, $id);
    ...
    public function getAll($where = '', $orderBy = null, $limit = null)
    {
        ...
        $res = $db->query('SELECT * FROM `' . $this->_table . '` ' . $extra);
        $result = array();
    ...
    public function getOne($where = '', $orderBy = null)
    {
        ...
        $row = $this->getDb()->run('SELECT * FROM `' . $this->_table . '` ' . $extra);
    ...
    public function getSelectionList($where = '', $orderBy = null)
    {
        ...
        return $this->getDb()->getList('id', $this->_listField, 'SELECT * FROM `' . $this->_table . '` ' . $extra);
    }
    ...
    /**
     * Create an object, based on class. Retrieves from memory cache if possible.
     * 
     * @param string $class Proper classname without Model_ in front of it.
     * @param array|int $values
     * @return Model A class, derived from model, based on $class.
     */
    public static function make($class, $values = null, $force = false)
    {
        if (empty($class))
            return null;
        $class = self::CLASS_PREFIX . $class;

        if (!$force && is_int($values)
                && !empty(self::$cache[$class])
                && !empty(self::$cache[$class][$values])
        ) {
            #Return the cached object.
            return self::$cache[$class][$values];
        } else {
            #Return a new object.
