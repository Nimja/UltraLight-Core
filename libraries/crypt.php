...
class Crypt
{
	...
	public static function encode($numbers)
	{
		if (empty($numbers) || !is_array($numbers)) {
			show_error('Trying to encode with empty array.');
			return FALSE;
		}
		...
		foreach ($numbers as $number) {
			if ($number <= 0 || !is_numeric($number)) {
				show_error('Data needs to be numbers.');
				return FALSE;
			}
	...
	public static function decode($string)
	{
		...
		if (!is_numeric($digits) && $digits > 0) {
			show_error('Improper string for decoding.');
			return FALSE;
		}
		...
		if (strlen($string) % $digits != 0) {
			show_error('String is incorrect length.');
			return FALSE;
		}
