...
class Image {
	...
	public function load($src)
	{
		...
		#Load image
		switch ($ext) {
			case 'png': $img = imagecreatefrompng($src) or show_error($src, 'PNG could not be loaded, corrupt?');
				break;
			case 'jpg':
			case 'jpeg': $img = imagecreatefromjpeg($src) or show_error($src, 'JPG could not be loaded, corrupt?');
				break;
			case 'gif': $img = imagecreatefromgif($src) or show_error($src, 'GIF could not be loaded, corrupt?');
				break;
		}
		$this->src = $src;
		$this->img = $img;
		if (empty($img)) {
			show_error($src, 'File is not an image.');
			return;
		}
