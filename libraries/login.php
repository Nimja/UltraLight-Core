...
class Login
{
    ...
    public static function logout()
    {
        ...
        Load::clearCookie(User::COOKIE_NAME);
        self::$user = null;
        self::$role = 0;
    }
    ...
    public static function loginBox($parse = true)
    {
        ...
        if ($loggedin) {
            ...
            $form = new Form();
            $form->begin('/login', 'post', array('id' => 'login_form'));
            ...
            if ($parse && $_SERVER['REQUEST_METHOD'] == 'POST') {
                if (empty($user) || empty($pass)) {
                    ...
                    if (empty($id)) {
                        $warning = 'Unknown username/password: ' . $pass;
                    } else {
                        $user = Model::make('user', $id);
